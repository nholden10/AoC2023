...
function determineRank(hand) {
  ...
  return rankHand(mostMatches, secondaryMatches, hand)
}

function adjustMatches(mostMatches, secondaryMatches, hand) {
  for (let i = 0; i < hand.length; i++) {
    if (hand[i] == 'J') {
      console.log(`Adjusting hand for Joker..`)
      mostMatches++
  ...
  return [mostMatches, secondaryMatches]
}

function rankHand(mostMatches, secondaryMatches, hand) {
  const [adjustedMost, adjustedSecondary] = adjustMatches(
    mostMatches,
    secondaryMatches,
    hand
  )
  mostMatches = adjustedMost
  secondaryMatches = adjustedSecondary
  let rank
  if (mostMatches == 5) {
